...
const commands = [];
const commandGroups = fs.readdirSync(path.join(__dirname, './commands')).filter(group=>group.isDirectory);
...
for (const group of commandGroups) {
	...
	for (const subgroup of commandSubGroups) {
		const commandFiles = fs.readdirSync(path.join(__dirname, `./commands/${group}/${subgroup}`)).filter(subgroup=>!file.isDirectory, file => file.endsWith('.js'));
