...
const commands = [];
const commandGroups = fs.readdirSync(path.join(__dirname, './commands'), {withFileTypes: true}).filter(group=>!group.isFile());

for (const group of commandGroups) {
	const commandSubGroups = fs.readdirSync(path.join(__dirname, `./commands/${group.name}`), {withFileTypes: true}).filter(subgroup=>!subgroup.isFile());
	const commandGroupFiles = fs.readdirSync(path.join(__dirname, `./commands/${group.name}`), {withFileTypes: true}).filter(subgroup=>subgroup.isFile(), file => file.endsWith('.js'));
		
	for (const groupFile of commandGroupFiles) {
		let command = require(path.join(__dirname, `./commands/${group.name}/${subgroup.name}/${groupFile.name}`));
	...
	for (const subgroup of commandSubGroups) {
		const commandFiles = fs.readdirSync(path.join(__dirname, `./commands/${group.name}/${subgroup.name}`), {withFileTypes: true}).filter(file=>file.isFile(), file => file.endsWith('.js'));
