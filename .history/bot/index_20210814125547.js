...
const commands = [];
const commandGroups = fs.readdirSync(path.join(__dirname, './commands'), {withFileTypes: true}).filter(group=>group.isDirectory);

for (const group of commandGroups) {
	const commandSubGroups = fs.readdirSync(path.join(__dirname, `./commands/${group}`)).filter(subgroup=>subgroup.isDirectory);
