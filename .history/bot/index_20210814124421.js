...
const commands = [];
const commandFolders = fs.readdirSync(path.join(__dirname, './commands')).filter(folder=>folder.isDirectory);
for (const folder of commandFolders) {
	const commandFiles = fs.readdirSync(path.join(__dirname, `./commands/${folder}`)).filter(file=>!file.isDirectory, file => file.endsWith('.js'));
